...
# Activar y desactivar herramientas — LGA ToolPack-B
...
Menu **TP2 > Enable Tools**. Se destilda lo que no se quiere y se guarda.
...
| Sistema | Archivo |
|---|---|
| Windows | `%APPDATA%\LGA\ToolPack_B\Enabled.ini` |
| macOS | `~/Library/Application Support/LGA/ToolPack_B/Enabled.ini` |
...
El instalador renombra la carpeta del pack a `LGA_ToolPack-B_backup` y copia la
...
1. inis historicos de los packs hermanos — hay tools que cambiaron de pack, y
   quien apago `CopyCat_Cleaner` o `Paste_To_Selected` cuando vivian en
   LGA_ToolPack tiene que seguir viendolos apagados ahora que viven aca;
...
- **Sin carpeta de datos del sistema:** la config va a
  `<.nuke>/LGA_Settings/ToolPack_B/Enabled.ini`. No se escribe sobre el ini
...
**`LGA_ToolPack-B/py/LGA_ToolPackB_Enabled.py`**
