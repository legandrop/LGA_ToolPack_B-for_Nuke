...
#                                                                                #
# 1.5 release notes:                                                             #
# - Now works in Nuke 16 and below.                                              #
# - Changed to match Nuke's default UI colour scheme.                            #
##################################################################################
...
if pyside_imported:
    ...
    nuke.menu("Animation").addCommand(
        "Animation Maker...",
        "AnimationMaker.showWindow()",
        "",
        icon="ParticleBounce.png",
    )
else:
    # create a dummy class to prevent an exception from the Class definitions
    # that are inheriting from QtGuiWidgets
    class QtGuiWidgets:
        @property
        def QWidget():
            return obj()
    ...
    nuke.tprint(
        "Unable to load AnimationMaker, so it was skipped: '{}'".format(error_message)
    )
...
class AnimationWindow(QtGuiWidgets.QWidget):
    ...
    def __init__(self, knobName, knobIndex=-1, parent=None):
        ...
        if self.editMode:
            ...
            try:
                self.animType = self.theNode[
                    "a_animType_{}".format(self.theKnobName)
                ].value()
            except Exception:
                self.animType = "ease"
        else:
        ...
        self.main_colour = (84, 118, 153)  # light blue (nuke default animation colour)
        self.alt_colour = (211, 67, 29)  # red (nuke default in/out markers colour)
        self.main_colour_style = "4D4D4D"  # light grey (nuke default button colour)
        self.bg_colour_style = "3A3A3A"  # dark grey (nuke default background colour)
        ...
        self.closeButton.setStyleSheet(
            "QWidget { background-color: #%s }" % self.main_colour_style
        )
        ...
        self.copyrightLabel.setText(
            '<p style="font-family:arial;color:gray;font-size:10px;">AnimationMaker 1.5 - David Emeny 2025</p>'
        )
        ...
        self.presetReloadButton.setGeometry(
            self.width / 2, self.presetBox.y() + 25, 89, 20
        )
        ...
        self.presetUpdateButton.setGeometry(
            self.width / 2 + 97, self.presetBox.y() + 25, 85, 20
        )
        ...
        self.presetDeleteButton.setGeometry(
            self.width / 2 + 190, self.presetBox.y() + 25, 88, 20
        )
        ...
        self.view.setGeometry(
            0, self.viewYoffset, self.width, self.height - self.viewYoffset
        )
        ...
        self.scene.setSceneRect(
            QtCore.QRect(0, 0, self.view.width(), self.view.height())
        )
        ...
        self.plotline1 = lineGraphic(
            self,
            self.plot_xMin,
            self.plot_yMin,
            self.plot_xMin,
            self.plot_yMax,
            QtGui.QColor(*self.alt_colour),
        )
        self.scene.addItem(self.plotline1)

        self.plotline2 = lineGraphic(
            self,
            self.plot_xMax,
            self.plot_yMin,
            self.plot_xMax,
        ...
        self.slidingBall = Particle(
            self, 8, [self.slidingBall_xMin, self.slidingBall_yMin]
        )
        ...
        self.growingBall.move(
            [self.width / 6, self.viewTop + ((self.boxheight - self.viewTop) / 2)]
        )
        ...
        self.flashingBall.move(
            [(self.width / 6) * 5, self.viewTop + ((self.boxheight - self.viewTop) / 2)]
        )

        # set up graphics text
        start_frame = int(float(self.startFrame.text()))
        end_frame = int(float(self.endFrame.text()))
        ...
        self.startText.setText("%g" % start_frame)
        ...
        self.endText.setText("%g" % end_frame)
        ...
        self.midText.setText("%g" % end_frame)
    ...
    def timerEvent(self, e):
        if self.timerState == "head":
            ...
            if (self.frameCounter >= int(float(self.endFrame.text()))) and (
                self.animType == "ease"
            ):
                self.timer.stop()
                self.startTail()
                ...
                try:
                    ...
                    a = (curveValue * c) + b
                    a = min(
                        max(a, self.flashingBall_aMin), self.flashingBall_aMax
                    )  # clamp
    ...
    def beginAnimations(self):
        ...
        start_frame_old = int(float(self.startFrameOld))
        end_frame_old = int(float(self.endFrameOld))

        self.frameCounter = start_frame_old
        self.duration = 1 + end_frame_old - start_frame_old  # in fps
    ...
    def createControlsForKnob(self):
        ...
        theTab = nuke.Tab_Knob(
            "anim_tab_" + self.theKnobName, self.theKnobName + " anim"
        )
        self.theNode.addKnob(theTab)

        # make ease controls

        easeType = nuke.Text_Knob(
            "a_easeType_display_" + self.theKnobName, "Animation type:"
        )
        ...
        startValue = nuke.Double_Knob(
            "a_startValue_" + self.theKnobName, "Start Value:"
        )
        ...
        waveType = nuke.Text_Knob(
            "a_waveType_display_" + self.theKnobName, "Animation type:"
        )
        ...
        op_choice = nuke.Enumeration_Knob(
            "a_opchoice_" + self.theKnobName, "", ["Mix back", "Add"]
        )
        ...
        base_choice = nuke.Enumeration_Knob(
            "a_basechoice_" + self.theKnobName,
            "",
            ["Equilibrium", "Min value", "Max value", "Orig value/curve"],
        )
        ...
        origValHidden = nuke.Text_Knob(
            "a_origVal_" + self.theKnobName, "Original value"
        )
...
if tab_knob:
    try:
        ...
        editButton.setValue(
            "AnimationMaker.showWindow('%s',%s)" % (self.origKnobName, self.knobIndex)
        )
        editButton.setTooltip(
            "Open the interface again to edit the wave type and other options."
        )
        ...
        removeButton.setValue(
            "AnimationMaker.remove_tab('%s','%s','%s',%s)"
            % (self.theNode.name(), theTab.name(), self.origKnobName, self.knobIndex)
        )
        removeButton.setTooltip(
            "Remove this tab and all its knobs, and create an expression using the current values."
        )
        ...
        restoreButton.setValue(
            "AnimationMaker.remove_tab('%s','%s','%s',%s,'%s')"
            % (
                self.theNode.name(),
                theTab.name(),
                self.origKnobName,
        ...
        restoreButton.setTooltip(
            "Remove this tab and all its knobs and restore the original value/curve."
        )
    ...
    def closeButtonPressed(self):
        ...
        if self.animType == "ease" or self.animType == "waveEase":
            ...
            self.theNode["a_startFrame_" + self.theKnobName].setValue(
                int(float(self.startFrame.text()))
            )
            self.theNode["a_endFrame_" + self.theKnobName].setValue(
                int(float(self.endFrame.text()))
            )
            self.theNode["a_startValue_" + self.theKnobName].setValue(startValueFloat)
            self.theNode["a_startValue_" + self.theKnobName].setRange(
                startValueFloat - (rangeFloat / 4), endValueFloat
            )
            self.theNode["a_endValue_" + self.theKnobName].setValue(endValueFloat)
            self.theNode["a_endValue_" + self.theKnobName].setRange(
                startValueFloat, endValueFloat + (rangeFloat / 4)
            )
            self.theNode["a_easeType_" + self.theKnobName].setValue(
                str(self.easeType.currentText())
            )
            self.theNode["a_easeType_display_" + self.theKnobName].setValue(
                "<h3><font color='yellow'>EASE </font><font color='white'>"
                + str(self.easeType.currentText())
                + "</font></h3>"
            )
            ...
            self.theNode["a_baseline_" + self.theKnobName].setValue(
                ((endValueFloat - startValueFloat) / 2) + startValueFloat
            )

            # update the basechoice dropdown
            self.theNode["a_basechoice_" + self.theKnobName].setValues(
                ["Equilibrium", "Start value", "End value", "Orig value/curve"]
            )
        ...
        if self.animType == "wave" or self.animType == "waveEase":
            ...
            self.theNode["a_minValue_" + self.theKnobName].setRange(
                minValueFloat - (rangeFloat / 4), maxValueFloat
            )
            self.theNode["a_maxValue_" + self.theKnobName].setValue(maxValueFloat)
            self.theNode["a_maxValue_" + self.theKnobName].setRange(
                minValueFloat, maxValueFloat + (rangeFloat / 4)
            )

            self.theNode["a_wavelength_" + self.theKnobName].setValue(wavelengthFloat)
            self.theNode["a_wavelength_" + self.theKnobName].setRange(
                wavelengthFloat - (wavelengthFloat / 4),
                wavelengthFloat + (wavelengthFloat * 4),
            )
            self.theNode["a_offset_" + self.theKnobName].setValue(offsetFloat)
            self.theNode["a_offset_" + self.theKnobName].setRange(
                offsetFloat - (offsetFloat * 0.25) - 10,
                offsetFloat + (offsetFloat * 4) + 10,
            )
            ...
            self.theNode["a_waveType_" + self.theKnobName].setValue(
                str(self.waveType.currentText())
            )
            if self.squarifyOld == True:
                self.theNode["a_waveType_display_" + self.theKnobName].setValue(
                    "<h3><font color='yellow'>WAVE </font><font color='white'>"
                    + str(self.waveType.currentText())
                    + " (squared)</font></h3>"
                )
            ...
            # update the baseline value
            self.theNode["a_baseline_" + self.theKnobName].setValue(
                ((maxValueFloat - minValueFloat) / 2) + minValueFloat
            )
        ...
        if "a_animType_%s" % (self.theKnobName) in theNodeKnobs:
            ...
            animTypeHidden = nuke.Text_Knob(
                "a_animType_" + self.theKnobName, "Anim type:"
            )
        ...
        if "a_exprTextAdd_%s" % (self.theKnobName) in theNodeKnobs:
            self.theNode["a_exprTextAdd_" + self.theKnobName].setValue(
                self.theExpression[1]
            )
        else:
            expAddHidden = nuke.Text_Knob(
                "a_exprTextAdd_" + self.theKnobName, "Add expression:"
            )
        ...
        if "a_exprTextMult_%s" % (self.theKnobName) in theNodeKnobs:
            self.theNode["a_exprTextMult_" + self.theKnobName].setValue(
                self.theExpression[2]
            )
        else:
            expMultHidden = nuke.Text_Knob(
                "a_exprTextMult_" + self.theKnobName, "Mult expression:"
            )
    ...
    def setUpEaseBoxes(self):
        ...
        if self.editMode:
            try:
                theIndex = self.easeType.findText(
                    self.theNode["a_easeType_" + self.theKnobName].value(),
                    QtCore.Qt.MatchExactly,
                )
        ...
        if self.editMode:
            try:
                self.startFrame.setText(
                    str(int(self.theNode["a_startFrame_" + self.theKnobName].value()))
                )
            except Exception:
        ...
        if self.editMode:
            try:
                self.endFrame.setText(
                    str(int(self.theNode["a_endFrame_" + self.theKnobName].value()))
                )
            except Exception:
        ...
        if self.editMode:
            try:
                self.startValue.setText(
                    "%g" % self.theNode["a_startValue_" + self.theKnobName].value()
                )
            except Exception:
        ...
        if self.editMode:
            try:
                self.endValue.setText(
                    "%g" % self.theNode["a_endValue_" + self.theKnobName].value()
                )
            except Exception:
    ...
    def setUpWaveBoxes(self):
        ...
        if self.editMode:
            try:
                theIndex = self.waveType.findText(
                    self.theNode["a_waveType_" + self.theKnobName].value(),
                    QtCore.Qt.MatchExactly,
                )
        ...
        if self.editMode:
            try:
                self.wavelength.setText(
                    "%g" % self.theNode["a_wavelength_" + self.theKnobName].value()
                )
            except Exception:
        ...
        if self.editMode:
            try:
                self.offset.setText(
                    "%g" % self.theNode["a_offset_" + self.theKnobName].value()
                )
            except Exception:
        ...
        if self.editMode:
            try:
                self.minValue.setText(
                    "%g" % self.theNode["a_minValue_" + self.theKnobName].value()
                )
            except Exception:
        ...
        if self.editMode:
            try:
                self.maxValue.setText(
                    "%g" % self.theNode["a_maxValue_" + self.theKnobName].value()
                )
            except Exception:
        ...
        self.waveEase = QtGuiWidgets.QRadioButton(
            "Combine with ease curve", self.radioGroup
        )
        ...
        if self.editMode:
            try:
                if (
                    "squared"
                    in self.theNode["a_waveType_display_" + self.theKnobName].value()
                ):
        ...
        if self.editMode:
            try:
                self.bcutoff.setText(
                    "%g" % self.theNode["a_bcutoff_" + self.theKnobName].value()
                )
            except Exception:
    ...
    def userValuesChanged(self):
        ...
        if self.checkForChangedValues():
            ...
            self.duration = 1 + int(float(self.endFrame.text())) - int(float(self.startFrame.text()))
    ...
    def plotCurve(self):
        ...
        if self.animType == "ease" or self.animType == "waveEase":
            ...
            current_start_frame = int(float(self.startFrame.text()))
            current_end_frame = int(float(self.endFrame.text()))

            if self.animType == "waveEase":
                self.startDot.move([self.plot_xMin, self.plot_yMin])
                self.endDot.move(
                    [(self.plot_xMax + self.plot_xMin) / 2, self.plot_yMax]
                )
                self.plotline3.setLine(
                    (self.plot_xMax + self.plot_xMin) / 2,
                    self.plot_yMin,
                    (self.plot_xMax + self.plot_xMin) / 2,
                    self.plot_yMax,
                )
                ...
                self.startText.setText("%g" % current_start_frame)
                self.midText.setText("%g" % current_end_frame)
                ...
                self.endDot.move([self.plot_xMax, self.plot_yMax])
                self.startText.setText("%g" % current_start_frame)
                self.endText.setText("%g" % current_end_frame)
                ...
                if index > 0:
                    if not dotsExist:
                        theLine = lineGraphic(
                            ...
                            plotX,
                            plotY,
                            self.dots[index - 1].pos[0],
                            self.dots[index - 1].pos[1],
                            QtGui.QColor(*self.main_colour),
                        )
                        self.lines.append(theLine)
                        self.scene.addItem(theLine)
                    else:
                        self.lines[index - 1].setLine(
                            plotX,
                            plotY,
                            self.dots[index - 1].pos[0],
                            self.dots[index - 1].pos[1],
    ...
    def getEaseExpression(self, forPanel=False):
        ...
        if forPanel == True:
            ...
            easePrefix = "frame < %s ? 0 : frame > %s ? 1 : " % (
                self.startFrameOld,
                self.endFrameOld,
            )
            ...
            if self.animType == "waveEase":
                ...
                b = "0"
                easePrefix = (
                    "frame < a_startFrame_%s ? 0 : frame > a_endFrame_%s ? 1 : "
                    % (self.theKnobName, self.theKnobName)
                )
            else:
                easePrefix = (
                    "frame < a_startFrame_%s ? a_startValue_%s : frame > a_endFrame_%s ? a_endValue_%s : "
                    % (
                        self.theKnobName,
                        self.theKnobName,
                        self.theKnobName,
                ...
                c = "(a_endValue_%s - a_startValue_%s)" % (
                    self.theKnobName,
                    self.theKnobName,
                )
            ...
            d = "(a_endFrame_%s - a_startFrame_%s)" % (
                self.theKnobName,
                self.theKnobName,
            )
            td = "((frame - a_startFrame_%s) / (a_endFrame_%s - a_startFrame_%s))" % (
                self.theKnobName,
                self.theKnobName,
                self.theKnobName,
            )
            td2 = (
                "((frame - a_startFrame_%s) / ((a_endFrame_%s - a_startFrame_%s) / 2))"
                % (self.theKnobName, self.theKnobName, self.theKnobName)
            )
        ...
        if theEaseType == "Linear":
            ...
            theExpression = (
                easePrefix
                + "(%s < 1 ? %s/2 * %s * %s + %s : -%s/2 * ((%s-1)*((%s-1)-2) - 1) + %s)"
                % (td2, c, td2, td2, b, c, td2, td2, b)
            )

        elif theEaseType == "Expo Ease IN":
            # c * (2 ** (10 * (td - 1)) ) + b
            theExpression = easePrefix + "%s * (2 ** (10 * (%s - 1)) ) + %s" % (
                c,
                td,
                b,
            )
            ...
            theExpression = (
                easePrefix
                + "%s < 1 ? %s / 2 * (2 ** (10 * (%s - 1)) ) + %s : %s / 2 * (-(2 ** (-10 * (%s-1))) + 2) + %s"
                % (td2, c, td2, b, c, td2, b)
            )
            ...
            theExpression = (
                easePrefix
                + "%s * ((%s/%s-1)*(%s/%s-1)*((%s+1)*(%s/%s-1) + %s) + 1) + %s"
                % (c, t, d, t, d, 1.70158, t, d, 1.70158, b)
            )
            ...
            theExpression = (
                easePrefix
                + "%s < (1/2.75) ? %s * (7.5625 * %s * %s) + %s : %s < (2/2.75) ? %s * (7.5625 * (%s - (1.5/2.75)) * (%s - (1.5/2.75)) + 0.75) + %s :        %s < (2.5/2.75) ? %s * (7.5625 * (%s - (2.25/2.75)) * (%s - (2.25/2.75)) + 0.9375) + %s : %s * (7.5625 * (%s - (2.625/2.75)) * (%s - (2.625/2.75)) + 0.984375) + %s"
                % (td, c, td, td, b, td, c, td, td, b, td, c, td, td, b, c, td, td, b)
            )
            ...
            theExpression = (
                easePrefix
                + "%s * (2**(-10 * %s)) * sin( (%s * %s-((%s * 0.3) / 4)) * (3.14159 * 2) / (%s * 0.3) ) + %s + %s"
                % (c, td, td, d, d, d, c, b)
            )
    ...
    def getWaveExpression(self, forPanel=False, easeExpression=None):
        ...
        if theWaveType == "Sine":
            ...
            theExpression = (
                "min(max(((fBm((frame/%s)+%s, 0, 0,10,2,0.5)*0.75)+0.5),0),1)" % (w, o)
            )
            ...
            theExpression = (
                "max(min((turbulence((frame/%s)+%s, 0, 0,10,2,0.5)*(1+1/3)),1),0)"
                % (w, o)
            )

        elif theWaveType == "Triangle":
            # (((((2*asin(sin(2*pi*(frame/waveLength)+offset)))/pi) / 2)+0.5) * (maxVal-minVal) ) + minVal
            theExpression = "((((2*asin(sin(2*pi*(frame/%s)+%s)))/pi) / 2)+0.5)" % (
                w,
                o,
            )
            ...
            theExpression = (
                ...
                + ")/("
                + w
                + "/2.46666666)) % (pi/2)))>0.99999? 1 : (sin((1/(pi/2))*(((frame-1)+"
                + o
                + ")/("
                + w
                + "/2.46666666)) % (pi/2)))"
            ...
            theExpression = (
                ...
                + ")/("
                + w
                + "/2.46666666)) % (pi/2)))>0.99999? 1 : (cos((1/(pi/2))*(((frame-1)+"
                + o
                + ")/("
                + w
                + "/2.46666666)) % (pi/2)))"
            ...
            theExpression = (
                ...
                + ")/("
                + w
                + "/4.934802)) % pi*2)))/534.5)) - 0.00186741)>0.999987? 1 : (((((exp((1/(pi/2))*(((frame-1)+"
                + o
                + ")/("
                + w
                + "/4.934802)) % pi*2)))/534.5)) - 0.00186741)"
            )
            ...
            theExpression = (
                "(sin(((frame/%s)*pi)+%s)>0?sin(((frame/%s)*pi)+%s):cos((((frame/%s)*pi)+%s)+(pi/2)))"
                % (w, o, w, o, w, o)
            )
        ...
        if self.squarifyOld == True:
            if forPanel:
                ...
                c05 = "(((a_maxValue_%s - a_minValue_%s)/2)+a_minValue_%s)" % (
                    self.theKnobName,
                    self.theKnobName,
                    self.theKnobName,
                )
    ...
    def save_preset_to_file(self):
        ...
        if success:
            ...
            msgBox.setStyleSheet(
                "QWidget { background-color: #%s }" % self.bg_colour_style
            )
...
class Particle:
    ...
    def __init__(self, graphicsWindow, rad, pos):
        ...
        self.graphic = QtGuiWidgets.QGraphicsEllipseItem(
...
def remove_tab(node_name, tab_name, knob_name, knob_index, orig_value=None):
    ...
    if orig_value:
        ...
        for k in reversed(to_remove):
            if k.Class() not in ["PyScript_Knob", "Tab_Knob", "Text_Knob"]:
                if k.isAnimated():
                    if k.hasExpression():
                        bake_values.append(
                            (k.name(), "(%s)" % k.animation(0).expression())
                        )
                    else:
                        has_keyframes = True
                        break
        ...
        if has_keyframes:
            ...
            if not nuke.ask(
                "There are keyframes involved in creating this curve, so it must be baked if you want to remove the tab. Continue?"
            ):
                return None
...
class MyLineEdit(QtGuiWidgets.QLineEdit):
    ...
    def keyReleaseEvent(self, event):
        ...
        if self in [
            self.parent.wavelength,
            self.parent.offset,
            self.parent.startFrame,
            self.parent.endFrame,
        ]:
...
def get_preset_path():
    try:
        ...
        filepath = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "AnimationMaker_presets.xml"
        )
        return filepath
    except Exception:
        return None
