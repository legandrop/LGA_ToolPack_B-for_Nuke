...
____________________________________________________________________
  ...
  LGA_ToolPackB_Enabled v1.05 | Lega
  ...
  El principal de esta tool es LGA_ToolPackB_EnabledPanel.py, y los dos
...
LEGACY_INI_NAME = "_LGA_ToolPack-B_Enabled.ini"
...
def get_nuke_dir():
    ...
    El pack siempre vive en `<NukeDir>/LGA_ToolPack-B`, asi que el padre de
...
def get_user_path(create_dir=False):
    ...
    if create_dir:
        try:
            ...
            _warn("LGA ToolPack-B: no se pudo crear %s (%s)" % (user_dir, error))
            return None
...
def _read_ini(path, warn_if_unreadable=False):
    ...
    try:
        ...
        if warn_if_unreadable:
            ...
            _warn("LGA ToolPack-B: no se pudo leer %s (%s)" % (path, error))
        return {}
    ...
    try:
        ...
        if warn_if_unreadable:
            _warn("LGA ToolPack-B: no se pudo interpretar %s (%s)" % (path, error))
        return {}
...
def write_user_overrides(flags):
    ...
    if not defaults:
        ...
        _warn(
            "LGA ToolPack-B: no se pudo leer %s, no se guarda la configuracion "
    ...
    lines = [
        "; LGA ToolPack-B - Tools desactivadas por el usuario.",
        ";",
    ...
    try:
        ...
        _warn("LGA ToolPack-B: no se pudo guardar %s (%s)" % (path, error))
